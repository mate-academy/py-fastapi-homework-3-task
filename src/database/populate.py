...
class CSVDatabaseSeeder:
    ...
    def _preprocess_csv(self):
        ...
        data['orig_lang'] = data['orig_lang'].str.replace(r'\s+', '',
                                                          regex=True)
    ...
    def _get_or_create_bulk(self, model, items: list, unique_field: str):
        ...
        existing_dict = {getattr(item, unique_field): item for item in
                         existing}
    ...
    def seed(self):
        try:
            ...
            genres = set(
                genre.strip()
                for genres in data['genre'].dropna() for genre in
                genres.split(',')
                if genre.strip()
            )
            actors = set(
            ...
            languages = set(
                lang.strip()
                for langs in data['orig_lang'].dropna() for lang in
                langs.split(',')
                if lang.strip()
            )

            country_map = self._get_or_create_bulk(CountryModel, countries,
                                                   'code')
            genre_map = self._get_or_create_bulk(GenreModel, list(genres),
                                                 'name')
            actor_map = self._get_or_create_bulk(ActorModel, list(actors),
                                                 'name')
            language_map = self._get_or_create_bulk(LanguageModel,
                                                    list(languages), 'name')
            ...
            for i, (_, row) in enumerate(
                    tqdm(data.iterrows(), total=data.shape[0],

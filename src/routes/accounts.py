...
def activate(
    ...
    if activation_token.expires_at.replace(tzinfo=timezone.utc) < datetime.now(
            timezone.utc):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Invalid or expired activation token."
...
def request_password_reset(
    ...
    try:
        existing_reset_passwords = db.query(PasswordResetTokenModel).filter_by(
            user_id=user.id).all()
...
def complete_password_reset(
    ...
    reset_password = db.query(PasswordResetTokenModel).filter_by(
    ...
    if reset_password.expires_at.replace(tzinfo=timezone.utc) < datetime.now(
            timezone.utc):
        if reset_password:
            db.delete(reset_password)
            db.commit()
...
def refresh(
    ...
    token_exist = db.query(RefreshTokenModel).filter_by(
        token=user_data.refresh_token).first()

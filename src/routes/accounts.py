...
def register(
    ...
    try:
        ...
        if not group:
            ...
            db.refresh(group)
        new_user = UserModel(
            email=email, password=password, group_id=group.id, group=group
        )
        db.add(new_user)
        db.flush()
        db.refresh(new_user)
...
def activate(data: UserActivationRequestSchema, db: Session = Depends(get_db)):
    ...
    if not activation_token or datetime.now(
        timezone.utc
...
def create_reset_token(
    data: PasswordResetRequestSchema, db: Session = Depends(get_db)
):
...
def password_reset(
    ...
    if (
        not expected_token
        or expected_token.token != data.token
        or datetime.now(timezone.utc)
        > expected_token.expires_at.replace(tzinfo=timezone.utc)
    ):
        if expected_token:

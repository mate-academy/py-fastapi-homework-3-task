...
def register(user_data: UserRegistrationRequestSchema, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if not group:
            ...
            db.flush()
            db.refresh(group)
        new_user = UserModel(email=email, password=password, group_id=group.id, group=group)
        db.add(new_user)
        db.flush()
        db.refresh(new_user)
...
def login(
    ...
    try:
        ...
        refresh_token_obj = RefreshTokenModel(user_id=user.id, token=refresh_token_str)
